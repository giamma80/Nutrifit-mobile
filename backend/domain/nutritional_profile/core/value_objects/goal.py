...
class Goal(str, Enum):
    ...
    def calorie_adjustment(self, tdee: float) -> float:
        ...
        Returns static target calories for guidance. For dynamic tracking,
        use target_deficit() instead.
        ...
        Example:
            ...
            2000.0
        """
        adjustments = {Goal.CUT: -500, Goal.MAINTAIN: 0, Goal.BULK: +300}
        return tdee + adjustments[self]

    def target_deficit(self) -> float:
        """Get target daily calorie deficit/surplus.

        This is the PRIMARY metric for goal tracking. The actual daily
        balance (consumed - burned) should match this target.

        Returns:
            float: Target deficit (negative=cut, 0=maintain, positive=bulk)

        Example:
            >>> Goal.CUT.target_deficit()
            -500  # 500 kcal deficit per day
            >>> Goal.BULK.target_deficit()
            300   # 300 kcal surplus per day
        """
        return {
            Goal.CUT: -500,  # 500 kcal deficit
            Goal.MAINTAIN: 0,  # No deficit/surplus
            Goal.BULK: +300,  # 300 kcal surplus
        }[self]
