...
class UserData:
    ...
    def __post_init__(self) -> None:
        ...
        if not (30.0 <= self.weight <= 300.0):
            raise InvalidUserDataError(f"Weight must be 30-300 kg, got {self.weight}")

        if not (100.0 <= self.height <= 250.0):
            raise InvalidUserDataError(f"Height must be 100-250 cm, got {self.height}")

        if not (18 <= self.age <= 120):
            raise InvalidUserDataError(f"Age must be 18-120 years, got {self.age}")

        if self.sex not in ("M", "F"):
            raise InvalidUserDataError(f"Sex must be 'M' or 'F', got {self.sex}")
