...
class WeightForecast:
    ...
    def __post_init__(self) -> None:
        ...
        n = len(self.dates)
        if not (
            len(self.predictions) == n
            and len(self.lower_bound) == n
            and len(self.upper_bound) == n
        ):
            raise ValueError(
...
class WeightForecastService:
    ...
    def _forecast_arima(
        ...
        last_date = dates[-1]
        future_dates = [
            last_date + timedelta(days=i + 1) for i in range(days_ahead)
        ]
    ...
    def _forecast_exponential_smoothing(
        ...
        try:
            ...
            residuals = fitted_model.fittedvalues - y[: len(
