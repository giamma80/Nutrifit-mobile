...
class NutritionCalculationService:
    ...
    def recompute_calories_from_macros(
        ...
        if not nutrients.is_calories_consistent(tolerance_pct):
            logger.debug(
                f"Calories inconsistent: {nutrients.calories} vs {calculated}, "
                f"correcting"
            )
            return calculated, True
    ...
    def calculate_daily_summary(
        ...
        if plan and total_nutrients.calories:
            targets = plan.macro_targets
            target_adherence = {
                "calories": (calories_in / targets.calories) * 100,
                "protein": (
                    (total_nutrients.protein or 0) / targets.protein_g
                ) * 100,
                "carbs": (
                    (total_nutrients.carbs or 0) / targets.carbs_g
                ) * 100,
                "fat": (
                    (total_nutrients.fat or 0) / targets.fat_g
                ) * 100,
            }
...
def get_nutrition_service() -> Optional[NutritionCalculationService]:
    ...
    from domain.nutrition.adapters.activity_adapter import ActivityDataAdapter  # noqa
