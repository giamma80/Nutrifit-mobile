...
@pytest.mark.asyncio
async def test_meal_entries_empty() -> None:
    _reset_repo()
...
@pytest.mark.asyncio
async def test_meal_entries_after_logging() -> None:
    _reset_repo()
    mutation1 = _q(
        """
        mutation {
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r1 = await ac.post("/graphql", json={"query": mutation1})
        id1 = r1.json()["data"]["logMeal"]["id"]
        r2 = await ac.post("/graphql", json={"query": mutation2})
...
@pytest.mark.asyncio
async def test_meal_entries_limit() -> None:
    _reset_repo()
    async with AsyncClient(app=app, base_url="http://test") as ac:
        for i in range(5):
            mutation = _q(
            ...
            await ac.post("/graphql", json={"query": mutation})
...
@pytest.mark.asyncio
async def test_meal_entries_idempotency_no_duplicate() -> None:
    _reset_repo()
    mutation = _q(
        """
        mutation {
...
@pytest.mark.asyncio
async def test_meal_entries_after_before_filters() -> None:
    _reset_repo()
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        resp_after = await ac.post("/graphql", json={"query": query_after})
        ...
        resp_before = await ac.post("/graphql", json={"query": query_before})
        ts_list_before = [e["timestamp"] for e in resp_before.json()["data"]["mealEntries"]]
        assert ts_list_before == [ts2, ts1]


@pytest.mark.asyncio
async def test_meal_entries_limit_normalization() -> None:
    _reset_repo()
