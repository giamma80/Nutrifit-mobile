...
async def test_activity_ingest_happy_path(
    ...
    data: Dict[str, Any] | None = resp.json()["data"]["activity"]["syncActivityEvents"]
...
async def test_activity_ingest_duplicates_second_batch(
    ...
    d1: Dict[str, Any] = r1.json()["data"]["activity"]["syncActivityEvents"]
    assert d1["accepted"] == 1 and d1["duplicates"] == 0
    r2: Response = await client.post("/graphql", json={"query": m2})
    d2: Dict[str, Any] = r2.json()["data"]["activity"]["syncActivityEvents"]
...
async def test_activity_ingest_normalization_duplicate_via_seconds(
    ...
    r1: Response = await client.post("/graphql", json={"query": m1})
    assert r1.json()["data"]["activity"]["syncActivityEvents"]["accepted"] == 1
    r2: Response = await client.post("/graphql", json={"query": m2})
    d2: Dict[str, Any] = r2.json()["data"]["activity"]["syncActivityEvents"]
...
async def test_activity_ingest_invalid_values(client: AsyncClient) -> None:
    ...
    d: Dict[str, Any] = r.json()["data"]["activity"]["syncActivityEvents"]
    assert d["accepted"] == 0
...
async def test_activity_ingest_auto_idempotency_key(
    ...
    r3: Response = await client.post("/graphql", json={"query": mutation_first})
    d3: Dict[str, Any] = r3.json()["data"]["activity"]["syncActivityEvents"]
    assert d3["accepted"] == 2 and d3["duplicates"] == 0
    auto_key = d3["idempotencyKeyUsed"]
    assert auto_key and auto_key.startswith("auto-")
    r4: Response = await client.post("/graphql", json={"query": mutation_second_same})
    d4: Dict[str, Any] = r4.json()["data"]["activity"]["syncActivityEvents"]
    assert d4["accepted"] == 2 and d4["duplicates"] == 0
    assert d4["idempotencyKeyUsed"] == auto_key
    r5: Response = await client.post("/graphql", json={"query": mutation_changed})
    d5: Dict[str, Any] = r5.json()["data"]["activity"]["syncActivityEvents"]
    assert d5["accepted"] == 0 and d5["duplicates"] == 1
    reasons: List[str] = [r["reason"] for r in d5["rejected"]]
    assert "CONFLICT_DIFFERENT_DATA" in reasons
    assert d5["idempotencyKeyUsed"].startswith("auto-")
    assert d5["idempotencyKeyUsed"] != auto_key
