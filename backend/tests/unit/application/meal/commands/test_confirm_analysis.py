...
def handler(mock_repository, mock_event_bus):
    return ConfirmAnalysisCommandHandler(
...
class TestConfirmAnalysisCommandHandler:
    ...
    async def test_confirm_all_entries(
        ...
        command = ConfirmAnalysisCommand(
            ...
            confirmed_entry_ids=[e.id for e in meal.entries]
        )
    ...
    async def test_confirm_some_entries(
        ...
        command = ConfirmAnalysisCommand(
            meal_id=meal.id,
            user_id=meal.user_id,
            confirmed_entry_ids=confirmed_ids
        )
    ...
    @pytest.mark.asyncio
    async def test_confirm_meal_not_found(
        self,
        handler,
        mock_repository
    ):
