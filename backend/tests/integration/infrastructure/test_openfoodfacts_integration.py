...
async def test_openfoodfacts_lookup_real_barcode():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert (
            product.nutrients.protein is not None and product.nutrients.protein >= 0
        ), "Should have protein"
        assert (
            product.nutrients.carbs is not None and product.nutrients.carbs > 0
        ), "Should have carbs"
        assert product.nutrients.fat is not None and product.nutrients.fat > 0, "Should have fat"
...
async def test_openfoodfacts_nutrient_extraction():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert nutrients.protein is not None and nutrients.protein >= 0, "Protein should be present"
        assert nutrients.carbs is not None and nutrients.carbs > 0, "Carbs should be present"
        assert nutrients.fat is not None and nutrients.fat > 0, "Fat should be present"
        assert nutrients.fiber is not None and nutrients.fiber >= 0, "Fiber should be present"
        assert nutrients.sugar is not None and nutrients.sugar >= 0, "Sugar should be present"
        assert nutrients.sodium is not None and nutrients.sodium >= 0, "Sodium should be present"

        # Verify quantities match reference (Nutella is high fat/sugar)
        assert (
            nutrients.fat is not None and nutrients.fat > 20
        ), "Nutella should have high fat content"
        assert (
            nutrients.sugar is not None and nutrients.sugar > 40
        ), "Nutella should have high sugar content"
...
async def test_openfoodfacts_metadata_extraction():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert (
            product.serving_size_g is not None and product.serving_size_g > 0
        ), "Should extract serving size"
...
async def test_openfoodfacts_energy_fallback():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert product is not None and product.nutrients is not None
        ...
        print("   - Fallback conversion works (kJ → kcal if needed)")
...
async def test_openfoodfacts_salt_to_sodium_fallback():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert product is not None and product.nutrients is not None
        assert (
            product.nutrients.sodium is not None and product.nutrients.sodium >= 0
        ), "Should have sodium (direct or converted from salt)"

        print("\n✅ OpenFoodFacts Salt→Sodium Fallback:")
        print(f"   - Sodium extracted: {product.nutrients.sodium:.1f}mg")
        print("   - Fallback conversion works (salt → sodium if needed)")
...
async def test_openfoodfacts_product_not_found():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        print("   - Returns None for invalid barcode")
...
async def test_openfoodfacts_circuit_breaker_configured():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        assert hasattr(
            client.lookup_barcode, "__wrapped__"
        ...
        print("   - Circuit breaker configured on lookup_barcode")
...
async def test_openfoodfacts_multiple_products():
    ...
    async with OpenFoodFactsClient() as client:
        ...
        for name, product in results.items():
            print(
                f"   - {name}: {product.nutrients.calories} kcal, "
