...
async def test_photo_analysis_workflow_success(client: AsyncClient) -> None:
    ...
    analysis_result = data["data"]["meals"]["analyzeMealPhoto"]
    ...
    confirm_result = data2["data"]["meals"]["confirmMealAnalysis"]
...
async def test_barcode_analysis_workflow_success(client: AsyncClient) -> None:
    ...
    assert "errors" not in data, f"GraphQL errors: {data.get('errors')}"
    result = data["data"]["meals"]["analyzeMealBarcode"]
...
async def test_meal_lifecycle_crud(client: AsyncClient) -> None:
    ...
    data = resp.json()
    assert "errors" not in data, f"GraphQL errors: {data.get('errors')}"
    meal_id_str = data["data"]["meals"]["analyzeMealPhoto"]["meal"]["id"]
    ...
    data3 = resp3.json()
    assert "errors" not in data3
    assert data3["data"]["meals"]["updateMeal"]["meal"]["notes"] == "Delicious homemade pasta"
    ...
    assert data4["data"]["meals"]["deleteMeal"]["mealId"] == meal_id_str
...
async def test_aggregate_search_meals_with_filters(client: AsyncClient) -> None:
    ...
    meal1_id = resp1.json()["data"]["meals"]["analyzeMealPhoto"]["meal"]["id"]
...
async def test_aggregate_daily_summary(client: AsyncClient) -> None:
    ...
    data1 = resp1.json()
    breakfast_result = data1["data"]["meals"]["analyzeMealPhoto"]
    ...
    data2 = resp2.json()
    lunch_result = data2["data"]["meals"]["analyzeMealBarcode"]
    ...
    data3 = resp3.json()
    dinner_result = data3["data"]["meals"]["analyzeMealPhoto"]
...
async def test_meal_history_pagination(client: AsyncClient) -> None:
    ...
    for i in range(5):
        ...
        resp = await client.post("/graphql", json={"query": mutation})
        data = resp.json()
        meal_id = data["data"]["meals"]["analyzeMealPhoto"]["meal"]["id"]
        entry_ids = [e["id"] for e in data["data"]["meals"]["analyzeMealPhoto"]["meal"]["entries"]]
...
async def test_confirm_analysis_partial_rejection(client: AsyncClient) -> None:
    ...
    meal_data = data1["data"]["meals"]["analyzeMealPhoto"]["meal"]
    ...
    assert "errors" not in data2
    result = data2["data"]["meals"]["confirmMealAnalysis"]
...
async def test_update_meal_validation_errors(client: AsyncClient) -> None:
    ...
    resp1 = await client.post("/graphql", json={"query": create_mutation})
    meal_id = resp1.json()["data"]["meals"]["analyzeMealPhoto"]["meal"]["id"]
    ...
    assert "errors" not in data2
    result = data2["data"]["meals"]["updateMeal"]
    ...
    assert "errors" not in data3
    error_result = data3["data"]["meals"]["updateMeal"]
...
async def test_idempotency_key_behavior(client: AsyncClient) -> None:
    ...
    result1 = data1["data"]["meals"]["analyzeMealPhoto"]["meal"]
    ...
    assert "errors" not in data2
    result2 = data2["data"]["meals"]["analyzeMealPhoto"]["meal"]
    ...
    assert "errors" not in data3
    result3 = data3["data"]["meals"]["analyzeMealPhoto"]["meal"]
