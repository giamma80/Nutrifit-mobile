...
class TestKalmanTDEEServiceInitialization:
    ...
    def test_init_negative_variance(self):
        ...
        with pytest.raises(
            ValueError,
            match="Initial variance must be non-negative"
        ):
            KalmanTDEEService(
                initial_tdee=2000.0,
    ...
    def test_init_negative_process_noise(self):
        ...
        with pytest.raises(
            ValueError,
            match="Process noise must be non-negative"
        ):
            KalmanTDEEService(
                initial_tdee=2000.0,
                process_noise=-10.0
    ...
    def test_init_negative_measurement_noise(self):
        ...
        with pytest.raises(
            ValueError,
            match="Measurement noise must be positive"
        ):
            KalmanTDEEService(
                initial_tdee=2000.0,
...
class TestKalmanTDEEServiceUpdate:
    ...
    def test_update_invalid_calories(self):
        ...
        with pytest.raises(
            ValueError,
            match="Consumed calories cannot be negative"
        ):
    ...
    def test_update_enforces_minimum_tdee(self):
        ...
        for _ in range(10):
            service.update(
                weight_kg=service.state.previous_weight + 0.5,
...
class TestKalmanTDEEServiceEdgeCases:
    ...
    def test_high_variance_scenario(self):
        ...
        service = KalmanTDEEService(
            initial_tdee=2000.0,
            measurement_noise=1.0  # High measurement noise
        )
