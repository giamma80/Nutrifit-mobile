...
class TestWeightForecast:
    ...
    def test_inconsistent_lengths(self):
        ...
        with pytest.raises(
            ValueError,
            match="All forecast arrays must have the same length"
        ):
            WeightForecast(
                dates=[date(2025, 1, 1)],
...
class TestWeightForecastServiceValidation:
    ...
    def test_mismatched_lengths(self):
        ...
        with pytest.raises(
            ValueError,
            match="Dates and weights must have same length"
        ):
            service.forecast(
    ...
    def test_insufficient_data(self):
        ...
        with pytest.raises(
            ValueError,
            match="Need at least 2 data points"
        ):
            service.forecast(
                dates=[date(2025, 1, 1)],
                weights=[80.0],
    ...
    def test_unsorted_dates(self):
        ...
        with pytest.raises(
            ValueError,
            match="Dates must be sorted in ascending order"
        ):
            service.forecast(
...
class TestWeightForecastServiceSimple:
    ...
    def test_simple_forecast_very_short_history(self):
        ...
        margin_day1 = (
            forecast.upper_bound[0] - forecast.lower_bound[0]
        ) / 2
        margin_day7 = (
            forecast.upper_bound[-1] - forecast.lower_bound[-1]
        ) / 2
    ...
    def test_simple_forecast_dates_are_correct(self):
        ...
        expected_dates = [
            date(2025, 1, 3) + timedelta(days=i) for i in range(30)
        ]
...
class TestWeightForecastServiceExponentialSmoothing:
    ...
    def test_exp_smoothing_medium_long_history(self):
        ...
        weights = [
            80.0 - 0.15 * i + np.random.normal(0, 0.2)
            for i in range(20)
        ]
...
class TestWeightForecastServiceARIMA:
    ...
    def test_arima_long_history(self):
        ...
        weights = [
            85.0 - 5.0 * (1 - np.exp(-i / 20)) + np.random.normal(0, 0.15)
            for i in range(40)
        ]
...
class TestWeightForecastServiceRealWorld:
    ...
    def test_realistic_deficit_scenario(self):
        ...
        np.random.seed(42)
        weights = [
            85.0 - 0.5 * (i / 7) + np.random.normal(0, 0.3)
            for i in range(56)
        ]
        ...
        assert forecast.predictions[-1] == pytest.approx(
            expected_final, abs=2.0
        )
