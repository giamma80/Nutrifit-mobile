...
# --- Scenario calorieCorrected=True su pasta ---


@pytest.mark.asyncio
async def test_calorie_corrected_true_case(
    client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("AI_MEAL_PHOTO_MODE", "gpt4v")
    ...
    mutation = _q(
        """
        mutation CC {
          analyzeMealPhoto(input:{photoId:\"cc_enforce\"}) {
            id dishName
            items { label calories calorieCorrected enrichmentSource }
          }
        }
        """
    )
    ...
    payload = resp.json()
    assert "errors" not in payload, payload.get("errors")
    items = payload["data"]["analyzeMealPhoto"]["items"]
    assert items[0]["label"] == "pasta"
    assert items[0]["calories"] is not None
    assert items[0]["calorieCorrected"] is True
    assert items[0]["enrichmentSource"] == "category_profile"
