...
def generate_prompt_v3(*, locale: str = "it") -> str:
    ...
    return (
        ...
        " 2. label: SEMPRE inglese, preferibilmente una parola, "
        "    usa nomenclatura USDA (es: eggs non egg, chicken non poultry, beef non meat). "
        "    Se necessario usa due parole per questi casi:"
        "    - parti specifiche o preparazioni: "
        "      egg white, egg whole, chicken breast, chicken thigh, potato boiled, "
        "      potato fried, tomato sauce"
        "    - alimenti da classificare crudi (es: eggplant, bell pepper, zucchini) "
        "    - alimenti non conditi o non trasformati (es: whole wheat bread, potato raw) "
        " 3. display_name: nome italiano dell'alimento (es: pollo, riso, pasta "
        "    o se possibile pollo arrosto, spaghetti alla carbonara, panino con prosciutto) "
        " 4. Max 5 items; confidence 0-1; quantity.unit 'g' o 'piece' "
        " 5. Per alimenti usa la forma base USDA quando generico: egg=eggs, "
        "   tomato=tomatoes, ma specifica quando necessario: egg white, chicken breast "
        ' 6. Se nessun cibo: {"dish_title":"","items":[]}'
        "ESEMPI label USDA corretti: "
        "eggs, chicken, beef, pork, salmon, una, rice, pasta, bread, cheese, "
        "milk, tomatoes, apples, bananas, broccoli, spinach, potatoes raw, onions, "
        "carrots, olive oil, butter, yogurt"
    )
